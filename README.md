...
<head>
    ...
    <style>
        ...
        .scan-overlay {
            ...
            width: 80vw; height: 80vw; max-width: 450px; max-height: 450px;
            border: 2px solid rgba(255, 255, 255, 0.5); border-radius: 24px;
        ...
        /* Estilos Corrigidos para Listas Claras */
        .card-list-item {
            background: #334155; border: 1px solid #475569; border-radius: 12px; padding: 12px;
            display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;
        }
        .card-list-item .title { font-weight: bold; color: white; font-size: 14px; }
        .card-list-item .subtitle { font-size: 11px; color: #94a3b8; }
        .card-list-item.active { border-color: #10b981; background: rgba(16, 185, 129, 0.1); }
...
<body>
        ...
        <div class="panel-content">
            ...
            <div id="view-procurar" class="tab-content active">
                <div class="space-y-4">
                    ...
                    <div class="card bg-primary/10 border-primary/30 p-4 rounded-xl border">
                        <div class="flex justify-between items-center mb-2"><label class="text-xs font-bold uppercase text-primary-light">Modo Caça</label><span id="hunt-status" class="text-xs font-mono text-primary-light"></span></div>
                        <div class="flex gap-2"><input type="text" id="hunt-target-id" class="input-dark text-sm p-2" placeholder="ID Alvo"><button id="hunt-toggle-btn" onclick="toggleHuntMode()" class="btn-action px-4 text-sm w-auto">Ativar</button></div>
                    </div>
            ...
            <div id="view-dashboard" class="tab-content">
                ...
                <div class="bg-slate-900 p-4 rounded-xl border border-slate-700 mb-4">
                    <h3 class="text-xs font-bold text-gray-400 uppercase mb-3 flex items-center gap-2"><i class="ri-pulse-line"></i> Ritmo de Trabalho (Bips/Hora)</h3>
                ...
                <div class="bg-slate-900 p-4 rounded-xl border border-slate-700 mb-6">
                    <h3 class="text-xs font-bold text-gray-400 uppercase mb-3 flex items-center gap-2"><i class="ri-layout-masonry-line"></i> Mapa de Calor (Zonas)</h3>
            ...
            <div id="view-inventario" class="tab-content">
                <h2 class="text-xl font-bold mb-4">Listas por Zona</h2>
            ...
            <div id="view-zonas" class="tab-content">
                <h2 class="text-xl font-bold mb-4">Selecionar Zona Ativa</h2>
    ...
    <script>
        const CONFIG = { STORAGE_KEY: 'natefy_pro_v15', WEBHOOK: 'https://mrxnxtxhxn-creator.app.n8n.cloud/webhook-test/df5b4afe-2fc4-4692-a80f-257aca92edf9' };
        ...
        function loadState() { const s = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEY)); if(s) { state = { ...state, ...s }; state.idsToFind = new Set(s.idsToFind); state.idDescriptions = new Map(s.idDescriptions); state.zoneData = new Map(s.zoneData.map(([k,v])=>[k,new Set(v)])); if(state.isFastMode) { document.getElementById('fast-mode-btn').classList.add('active'); document.querySelector('.toggle-indicator').style.transform='translateX(100%)'; document.querySelector('.toggle-indicator').style.background='#10b981'; } } }
        ...
        async function downloadFlow() { if(state.logs.length===0)return alert("Sem dados"); const c=document.getElementById('flowchart-canvas-container'); c.innerHTML=`<div style="padding:50px;background:#1E1B4B;color:white;text-align:center"><h1>Relatório Visual</h1><h2>${state.operator}</h2><div style="display:flex;justify-content:center;gap:20px;margin-top:50px"><div style="border:2px solid #10B981;padding:20px">SUCESSO: ${state.foundIds.length}</div><div style="border:2px solid #EF4444;padding:20px">ERROS: ${state.logs.filter(l=>l.status!=='SUCESSO').length}</div></div></div>`; const cv=await html2canvas(c); const a=document.createElement('a'); a.href=cv.toDataURL(); a.download='Fluxo.png'; a.click();}
        ...
        function updateDashboard() {
            ...
            // SPH (Scans Per Hour - últimos 10 min)
            const tenMinAgo = new Date(Date.now() - 600000);
            const recent = state.logs.filter(l => new Date(l.time) > tenMinAgo).length;
            const sph = recent * 6; // Projeta para 1 hora
            
            // Tempo Médio (entre sucessos)
            let avgTime = 0;
            const sLogs = state.logs.filter(l=>l.status==='SUCESSO').sort((a,b)=>new Date(a.time)-new Date(b.time));
            if(sLogs.length > 1) {
                let sum = 0;
                for(let i=1; i<sLogs.length; i++) sum += (new Date(sLogs[i].time)-new Date(sLogs[i-1].time))/1000;
                avgTime = Math.round(sum/(sLogs.length-1));
            }
            ...
            // 2. Gráfico Ritmo (Line)
            const rCtx = document.getElementById('rhythmChart');
            if(rCtx) {
                if(state.charts.rhythm) state.charts.rhythm.destroy();
                const hourly = {};
                state.logs.forEach(l => { const h = new Date(l.time).getHours() + 'h'; hourly[h] = (hourly[h]||0)+1; });
                state.charts.rhythm = new Chart(rCtx, {
                    type: 'line',
                    data: { 
                        labels: Object.keys(hourly).sort(), 
                        datasets: [{ 
                            label: 'Bips/Hora', 
                            data: Object.keys(hourly).sort().map(k=>hourly[k]), 
                            borderColor: '#6366f1', 
                            backgroundColor: 'rgba(99,102,241,0.2)', 
                            fill: true, tension: 0.4 
                        }] 
                    },
                    options: { responsive: true, maintainAspectRatio: false, scales: { x: { display: true, grid: { display: false }, ticks: { color: '#94a3b8' } }, y: { display: false } }, plugins: { legend: { display: false } } }
                });
            }
